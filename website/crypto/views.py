...
from crypto.models import Coin, CoinForTable, Currency
...
def index(request):
    ...
    for x in all_cryptos:
        mapa.append({"id":i, "name": x.coin_name, "price_USD": x.price, "price_PLN": x.price * pln_course.rate, "percent_change_24h": x.percent_change_24h, "percent_change_7d": x.percent_change_7d, "percent_change_30d": x.percent_change_30d})
    ...
    data = []
    data2 = []
    name = all_cryptos.filter(coin_id = coin1_id).first().coin_name
    name2 = all_cryptos.filter(coin_id = coin2_id).first().coin_name
